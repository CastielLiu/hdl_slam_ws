...
public:
  /**
   * @brief constructor
   * @param system               system to be estimated  被估计的系统
   * @param state_dim            state vector dimension  状态向量维度
   * @param input_dim            input vector dimension  输入(控制)向量维度
   * @param measurement_dim      measurement vector dimension 测量(观测)向量维度
   * @param process_noise        process noise covariance (state_dim x state_dim) 过程噪声协方差
   * @param measurement_noise    measurement noise covariance (measurement_dim x measuremend_dim) 测量噪声协方差
   * @param mean                 initial mean  初始状态均值
   * @param cov                  initial covariance 初始状态协方差 
   */
  ...
  void predict(const VectorXt& control) {
    ...
    computeSigmaPoints(mean, cov, sigma_points); //根据均值和方差选取采样点
    for (int i = 0; i < S; i++) {
      sigma_points.row(i) = system.f(sigma_points.row(i), control); //根据控制指令更新采样点位置
    }
    ...
    cov_pred.setZero();
    for (int i = 0; i < S; i++) {
      mean_pred += weights[i] * sigma_points.row(i); //更新后采样点位置加权平均
    }
    for (int i = 0; i < S; i++) {
      VectorXt diff = sigma_points.row(i).transpose() - mean_pred;
      cov_pred += weights[i] * diff * diff.transpose();//更新后采样点加权协方差
    }
    cov_pred += R; //预测协方差添加过程噪声
